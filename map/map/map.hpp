...
namespace sjtu {
	...
	private:
		...
		node*root;
		node*GND;		//GND: replace NULL (ground)			//only GND's left and right is NULL			//GND->parent:max element	//�ӵ�
		int size_;
	...
	public:
		...
		iterator begin() { return iterator(GND->next, GND, root); }
		const_iterator cbegin() const { return const_iterator(GND->next, GND, root); }
	...
	private:
		bool insert(const value_type &value, node*&t, node*&newNode, node*last)
		{
			...
			else if (cmp(value.first, t->data->first))		//ȥ��������
			{
